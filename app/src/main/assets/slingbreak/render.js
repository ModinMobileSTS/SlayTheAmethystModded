(() => {
     ...
     if(['ready','flying'].includes(G.phase)){
      ...
      if(d&&d.dy>5){
        const len=Math.hypot(d.dx,d.dy),speed=G.speed()*(.56+.44*Math.min(1,len/100));
         const key=[Math.round(d.dx),Math.round(d.dy),G.predictionVersion,G.physicsStep].join(':');
        ...
        ctx.restore();travelled=0;
         path.forEach((p,i)=>{if(i>0)travelled+=Math.hypot(p.x-path[i-1].x,p.y-path[i-1].y);if(i%6===0){const fade=Math.min(1,Math.max(0,(PREDICTION_MAX-travelled)/(PREDICTION_MAX-PREDICTION_FADE_START)));ctx.globalAlpha=.85*fade;circle(p.x,p.y,2.8,predictionColor('#78a943'));}});ctx.globalAlpha=1;
      }else{
  ...
  function render(){
    ...
    for(const b of G.bricks){
      ...
      if(b.flash>0){ctx.globalAlpha=b.flash*3;rounded(b.x-b.w/2,b.y-b.h/2,b.w,b.h,5,'#fff');ctx.globalAlpha=1;}
      const ink={normal:'#75944f',bomb:'#9e4935',lightning:'#8f792e',frost:'#4f8d9e',prism:'#796099',gold:'#809142'}[b.type];
      if(b.type!=='normal'){
        ...
        if(showHp){
           const text=G.fmt(Math.ceil(b.hp)),baseSize=Math.min(22,b.h*.65),maxWidth=b.w/2-6;
          ctx.font=`500 ${baseSize}px "DM Sans", "Noto Sans SC", sans-serif`;
          const fontSize=baseSize*Math.min(1,maxWidth/Math.max(1,ctx.measureText(text).width));
          label(text,b.x+b.w/4,b.y+fontSize/3,fontSize,ink);
        }
      }
       else if(b.max>1){const size=Math.min(23,b.h*.65);label(G.fmt(Math.ceil(b.hp)),b.x,b.y+size/3,size,ink);}
      else{ctx.globalAlpha=.45;line(b.x-7,b.y,b.x+7,b.y,ink,1.1);ctx.globalAlpha=1;}
    ...
    for(const o of G.obstacles){
      ...
      if(o.flash>0){ctx.globalAlpha=o.flash*3;rounded(left,top,o.w,o.h,4,'#e7f5d3');ctx.globalAlpha=1;}
    }
    drawCore();
  ...
  const updatePointer=e=>{if(e.pointerType!=='touch'){G.pointerType=e.pointerType;G.pointerPos=point(e);}};
   const updateDrag=e=>{updatePointer(e);const p=point(e);let dx=p.x-G.origin.x,dy=p.y-G.origin.y;dy=Math.max(0,dy);const len=Math.hypot(dx,dy);if(len>105){dx*=105/len;dy*=105/len;}G.drag={dx,dy};const step=Math.floor(Math.min(100,Math.hypot(dx,dy))/20);if(step>drawStep)G.sound('draw',step);drawStep=step;document.querySelector('#power-readout b').textContent=Math.round(Math.min(100,Math.hypot(dx,dy)))+'%';};
  ...
  canvas.addEventListener('pointermove',e=>{if(G.drag&&G.pointer===e.pointerId)updateDrag(e);});
