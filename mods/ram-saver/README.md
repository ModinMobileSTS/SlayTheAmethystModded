...
Preserve the caller's GL texture binding while materializing a cold texture, maintain hot-pin accounting without scanning every historical texture state, apply sampler variants without mutating shared textures, and resolve fake textures around the original SpriteCache/PolygonRegion draw implementations. This addresses multi-texture sampling corruption, per-bind CPU overhead, unstable descriptor caches, and the regression where replacing SpriteCache partial draw with a raw method body made player card actions exceed the frame budget, including second-scale stalls. Type: rendering compatibility and performance fix implemented by the listed classes.
